...
class MediaListVC: UIViewController {
    ...
    override func viewWillDisappear(_ animated: Bool) {
        //  annnny()
        //  SQLiteManger.shared().createSearchHistoryTable2()
        //  SQLiteManger.shared().insertsearchtable(search: searchBar.text ?? "")
    ...
    //    func annnny(){
    //        if let sear = SQLiteManger.shared().getLastSearchText(){
    //            searchBar.text = sear
    //        }
    //    }
    //    func loadLastsearch(){
    //        if let sql = SQLiteManger.shared().getLastSearch() {
    //            print(sql.id)
    //            print(sql.searchDate)
    //            print(sql.searchText)
    //            let para: [String:String] = ["term" : sql.searchText]
    //            ApiManger.shared().getdata(mypara: para) { (error, artist) in
    //                if let error = error{
    //                    self.showAlert(msg: error.localizedDescription)
    //                }else if let arr = artist {
